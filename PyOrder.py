...
def get_Files():
    ...
    if mode == 'amazon':
        file_list = [
            file for file in allFiles if file.endswith(".txt")]
    elif mode == 'yahoo':
        file_list = [
            file for file in allFiles if file.endswith(".csv")]
    return file_list
...
def yahoo_files(file_list):
    ...
    fileProduct = [
        file for file in file_list if file.startswith("YahooProduct")]
    ...
    file_order = pd.read_csv('./'+orderFolder+'/' +
                             fileOrder, encoding="cp932",)
    file_product = pd.read_csv(
        './'+orderFolder+'/' + fileProduct, encoding="cp932",)
    file_order.fillna('', inplace=True)
    dic_order = file_order.set_index('Id').to_dict('index')

    df = file_product.loc[:, ['Id', 'ItemId',
    ...
    for key, value in dic_order.items():
        ...
        df.loc[df.Id == key,
               'addressModified'] = str(value['ShipPrefecture']) + str(value['ShipCity']) + str(value['ShipAddress1']) + str(value['ShipAddress2'])
...
def change_columns(df):
    if mode == 'yahoo':
        df.rename(columns={'Id': 'reference No.', 'ItemId': 'sku', 'Title': 'product-name', 'Quantity': 'quantity-purchased', 'UnitPrice': 'unit value', 'ShipName': 'Consignees NAME',
                  'ShipNameKana': 'Kana', 'ShipZipCode': 'ConsigneesPOST', 'addressModified': 'Consignees Address', 'ShipPhoneNumber': 'ConsigneesPhonenumber'}, inplace=True)
...
def get_address(df):
    ...
    df['address_modified'] = df['ship-state'] + df['ship-city'] + \
        df['ship-address-1']+" " + df['ship-address-2'] + \
        " " + +df['ship-address-3']
...
def input_data(df, code):
    for key, value in code.items():
        ...
        df.loc[df.sku == key, 'invoiceTotal'] = df['unit value'] * \
            df['quantity-purchased']
        ...
        if value['site'] == 'NZP' or value['site'] == 'YAH':
            prefix = 'IT'
...
def select_site(df):
    ...
    if site == 'NZP' or site == 'YAH':
        ...
        df['shipper address'] = '28 Huntington Park Drive Greenhithe Auckland 0632'
...
def output_file(df, site):
    columnNames = ['site', 'reference No.', 'tracking', 'bag No', 'Consignees NAME', 'Kana', 'ConsigneesPOST', 'Consignees Address', 'Consignees Address 2', 'ConsigneesPhonenumber', 'shipper Name',
                   'shipper address', 'shipper phone', 'term', 'invoiceTotal', 'number of parcels', 'weight', 'product name_eng', 'quantity-purchased', 'unit value', 'origin', 'code', 'product-name']

    df_output = df[columnNames]
    fileName = site+"_Amazon_Order_"+today.isoformat()+".xlsx"
    # 대문자로 바꾸기
    df_output.loc[:, 'product name_eng'] = df_output.loc[:,
                                                         'product name_eng'].str.upper()
